...
function NoteEdit() {
    ...
    const {id} = useParams();
    const currNote = notes.find(note=>(note._id==id));
    ...
    return (
        <div className=' min-w-full min-h-screen flex flex-wrap items-center justify-evenly bg-zinc-800 bg-opacity-70 fixed top-0'>
            <div className=" w-2/3 md:w-1/2 xl:w-1/3 p-4 my-3 flex flex-wrap justify-center rounded-lg" style={{backgroundColor:noteColor}} ref={ref}>
                ...
                <form onSubmit={handleSubmit} className='flex flex-wrap content-center items-center justify-evenly'>

                    <input type="text" defaultValue={currNote.title} name="title" id="title" placeholder='Title' className={`bg-transparent focus:outline-none p-2 text-2xl w-full text-white placeholder:text-white`}/>
